...
bool BeamTiming::isBeamTiming(double time) {
    for (int iBeam = 0; iBeam < nBeams; iBeam++) {
        if (arisings[iBeam] < time && time < fallings[iBeam]) {
            return true;
        }
    }
    return false;
...
void BeamTiming::GetBeamTimings(TString dir, bool figures = false, TString image = "image") {
	...
	timings . clear();
	TString cut = "height > 2000 && fwhm < 100";
	...
	for (int ibd = 1; ibd <= 9; ibd++) {
		...
		for (int ich = 0; ich < 16; ich++) {
			TTree *tree = (TTree *) file -> Get(Form("ch%i/pulse_ch%i", ich, ich));
			//TTree *tree = (TTree *) file -> Get(Form("ch%i/pul_ch%i", ich, ich));

			TH1D *h1_temp = new TH1D("h1_temp", "h1_temp;time (TDC); counts", 1024 / 8, 0, 4096);
			tree -> Draw("istime >> h1_temp", cut);
			//tree -> Draw("ZCtime1 >> h1_temp", cut);
			h1 -> Add(h1_temp);
	...
	for (int ibin = 0; ibin < maxbin - 1; ibin++) {
		...
		if ( x1 - timings.at(timings.size() - 1) > 100 && y1 > rms ) { 
	...
	for (int ibeam = 1; ibeam < timings.size(); ibeam++){
		...
		double start = mean - 5 * sigma;
		double end = mean + 5 * sigma;
	...
	h1 -> SetTitle(Form("%s nBeams : %i", image.Data(), nBeams));
	
	c1 -> Update();
	if ( figures ) { c1 -> SaveAs(Form("./beamtiming_%s.png", image.Data()), "png"); } 
	delete c1;
